...
/* istanbul ignore next */
MockBucket.prototype.enableN1ql = function() {
...
MockBucket.prototype.getMulti = function(keys, callback) {
  if (!Array.isArray(keys) || keys.length === 0) {
    var errmsg = 'First argument needs to be an array of non-zero length.';
...
MockBucket.prototype._store = function(key, value, options, callback, opType) {
  ...
  this._maybeInvoke(function() {
    ...
    if (opType === 'set') {
      ...
      if (origReplaceItem.lockExpiry && origReplaceItem.lockExpiry > new Date() 
        && !options.cas) {
        return callback(new CbError(
            'temporary error - key locked', errs.temporaryError), null);
      }
      ...
      if (origPrependItem.lockExpiry 
        && origPrependItem.lockExpiry > new Date()) {
        return callback(new CbError(
            'temporary error - key locked', errs.temporaryError), null);
      }
 ...
 * @param [exact] If both @c b and @c b are arrays, setting 
 * this parameter to true ensures that they will only be equal 
 * if their length matches and their contents match. If this 
 * value is false (the default), then only the common
...
MockBucket.prototype._execView = function(ddoc, name, options, callback) {
  this._indexView(ddoc, name, options, function(err, results, reducer) {
    ...
    if (options.skip && typeof options.skip !== 'number') {
      errmsg = 'query_parse_error: Invalid value for integer parameter: "' 
    ...
    if (reducer && options.reduce !== false) {
      ...
      for (var l = 0; l < results.length; ++l) {
        keyN = cbNormKey(results[l].key, group_level);
        if (cbIndexOf(reduceKeys, keyN) < 0) {
          reduceKeys.push(keyN);
        }
      }

      var reduceResults = [];
      for (var j = 0; j < reduceKeys.length; ++j) {
        var values = [];
        for (var k = 0; k < results.length; ++k) {
          keyN = cbNormKey(results[k].key, group_level);
          if (cbCompare(keyN, reduceKeys[j]) === 0) {
            values.push(results[k].value);
          }
        }
        var result = reducer(reduceKeys[j], values, false);
        reduceResults.push({
          key: reduceKeys[j],
          value: result
        });
