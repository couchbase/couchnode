...
function _defaultEncode(doc) {
  if (typeof doc === 'string') {
    return {
      flags: FLAGS.NF_UTF8,
      value: Buffer.from(doc, 'utf8'),
    };
  } else if (Buffer.isBuffer(doc)) {
    return {
      flags: FLAGS.NF_RAW,
      value: Buffer.from(doc),
    };
  } else {
    return {
      flags: FLAGS.NF_JSON,
      value: Buffer.from(JSON.stringify(doc), 'utf8'),
    };
  }
}
...
/* istanbul ignore next */
MockStorage.prototype.timeTravel = function(period) {
  this.timeOffset += period;
};
/* istanbul ignore next */
MockStorage.prototype._getDate = function() {
  var dt = new Date();
  dt.setTime(dt.getTime() + this.timeOffset);
  return dt;
};
/* istanbul ignore next */
MockStorage.prototype._makeExpiryDate = function(expiry) {
  if (!expiry) {
    return null;
  }
...
/* istanbul ignore next */
MockStorage.prototype._makeLockDate = function(lockTime) {
  var dt = this._getDate();
...
MockStorage.prototype.get = function(key, hashkey) {
  ...
  if (item.expiry && item.expiry <= this._getDate()) {
    return null;
  }
  return item;
};
...
/* istanbul ignore next */
MockBucket.prototype.timeTravel = function(period) {
  this.storage.timeTravel(period);
};
/* istanbul ignore next */
...
MockBucket.prototype._ensureConnected = function() {
  if (!this.connected) {
    throw new Error('cannot perform operations on a shutdown bucket');
  }
};

MockBucket.prototype.invalidateQueryCache = function() {
...
MockBucket.prototype.get = function(key, options, callback) {
  ...
  this._maybeInvoke(function() {
    ...
    var decValue = this._decodeDoc({
      value: origItem.value,
      flags: origItem.flags,
    });
    if (origItem.lockExpiry && origItem.lockExpiry > this.storage._getDate()) {
...
MockBucket.prototype.getMulti = function(keys, callback) {
  if (!Array.isArray(keys) || keys.length === 0) {
    throw new TypeError(
      'First argument needs to be an array of non-zero length.'
    );
  }
  if (typeof callback !== 'function') {
...
MockBucket.prototype.getAndTouch = function(key, expiry, options, callback) {
  ...
  this._maybeInvoke(function() {
    ...
    if (origItem.lockExpiry && origItem.lockExpiry > this.storage._getDate()) {
      return callback(
        new CbError('temporary error - key locked', errs.temporaryError),
        null
      );
    }
    origItem.expiry = this.storage._makeExpiryDate(expiry);
    var decValue = this._decodeDoc({
      value: origItem.value,
      flags: origItem.flags,
    });
...
MockBucket.prototype.getAndLock = function(key, options, callback) {
  ...
  this._maybeInvoke(function() {
    ...
    if (options.lockTime) {
      origItem.lockExpiry = this.storage._makeLockDate(options.lockTime);
    } else {
      origItem.lockExpiry = this.storage._makeLockDate(15);
    }
...
MockBucket.prototype.unlock = function(key, cas, options, callback) {
  ...
  this._maybeInvoke(function() {
    ...
    if (!_compareCas(origItem.cas, cas)) {
      return callback(
        new CbError('cas does not match', errs.keyAlreadyExists),
        null
      );
    }
    if (
      !origItem.lockExpiry ||
      origItem.lockExpiry <= this.storage._getDate()
    ) {
      return callback(new CbError('key not locked', errs.temporaryError), null);
    }
...
MockBucket.prototype._store = function(key, value, options, callback, opType) {
  ...
  this._maybeInvoke(function() {
    ...
    if (opType === 'set') {
      ...
      if (
        origSetItem &&
        origSetItem.lockExpiry &&
        origSetItem.lockExpiry > this.storage._getDate() &&
        !options.cas
      ) {
        return callback(
          new CbError('temporary error - key locked', errs.temporaryError),
      ...
      if (origSetItem && !_compareCas(origSetItem.cas, options.cas)) {
        return callback(
          new CbError('cas mismatch', errs.keyAlreadyExists),
          null
        );
      }
      ...
      var newSetItem = {
        ...
        expiry: this.storage._makeExpiryDate(options.expiry),
        cas: _createCas(),
      };
      ...
      if (origAddItem) {
        return callback(
          new CbError('key already exists', errs.keyAlreadyExists),
          null
        );
      }
      ...
      if (!origReplaceItem) {
        return callback(
          new CbError('key does not exist', errs.keyNotFound),
          null
        );
      }
      if (
        origReplaceItem.lockExpiry &&
        origReplaceItem.lockExpiry > this.storage._getDate() &&
        !options.cas
      ) {
        return callback(
          new CbError('temporary error - key locked', errs.temporaryError),
      ...
      if (
        origAppendItem.lockExpiry &&
        origAppendItem.lockExpiry > this.storage._getDate()
      ) {
        return callback(
          new CbError('temporary error - key locked', errs.temporaryError),
          null
      ...
      if (
        origPrependItem.lockExpiry &&
        origPrependItem.lockExpiry > this.storage._getDate()
      ) {
        return callback(
          new CbError('temporary error - key locked', errs.temporaryError),
          null
...
MockBucket.prototype.counter = function(key, delta, options, callback) {
  ...
  this._maybeInvoke(function() {
    ...
    if (options.initial !== undefined && !origCountItem) {
      var newCountItem = {
        value: Buffer.from(options.initial.toString(), 'utf8'),
        flags: 0,
        cas: _createCas(),
      };
    ...
    if (
      origCountItem.lockExpiry &&
      origCountItem.lockExpiry > this.storage._getDate()
    ) {
      return callback(
        new CbError('temporary error - key locked', errs.temporaryError),
        null
    ...
    numValue += delta;
    origCountItem.value = Buffer.from(numValue.toString(), 'utf8');
...
Object.defineProperty(MockBucket.prototype, 'name', {
  get: function() {
    return this._name;
  },
  writeable: false,
