 ...
 * @ignore
 */
Bucket.prototype._capiRequest = function(path, method) {
 ...
 * @ignore
 */
Bucket.prototype._mgmtRequest = function(path, method) {
...
/**
 * An event emitting allowing you to bind to various query result set
 * events.
 *
 * @constructor
 *
  ...
  req.on('response', function(resp) {
    ...
    if (resp.statusCode !== 200) {
      ...
      resp.on('end', function () {
        var jsonError = JSON.parse(errBuffer);
      ...
      if (events.EventEmitter.listenerCount(self, 'row')) {
        ...
        resp.on('end', function () {
          var jsonBuffer = JSON.parse(strBuffer);
          var meta = {
            total_rows: jsonBuffer.total_rows
          };
          self.emit('rows', jsonBuffer.rows, meta);
          self.emit('end', meta);
        });
      }
    }
...
Bucket.prototype._view = function(viewtype, ddoc, name, q, callback) {
  ...
  var httpReq = this._capiRequest(path, 'GET');
  var req = new ViewQueryResponse(httpReq, callback);
...
Bucket.prototype.query = function(query, params, callback) {
  ...
  if (query instanceof ViewQuery.Default) {
    ...
    return this._query(query.toString(params), callback);
  } else {
    throw new TypeError(
...
Bucket.prototype._invoke = function(fn, args) {
  try {
    ...
    args[args.length-1](new Error('lcb error ' + e.lcbError), null);
  }
};
 ...
 * Note that hashkey is an experimental feature.  It is not supported
 * and is not expected to work in some scenarios.
 *
