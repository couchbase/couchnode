...
/**
 * Retrieves a list of keys from a replica server in your cluster
 *
 * @param {Array.<Buffer|string>} keys
 * The target document keys.
 * @param {Object} [options]
 ...
 * @committed
 */
Bucket.prototype.getMultiReplica = function(keys, options, callback) {
  if (options instanceof Function) {
    callback = arguments[1];
    options = {};
  }
  ...
  function getOne() {
    var key = keys[sentCount++];
    self.getReplica(key, options,function(err, res) {
      resCount++;
      if (err) {
        errCount++;
        outMap[key] = { error: err };
