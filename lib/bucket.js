 ...
 * @private
 */
function Bucket(options, callback) {
  var lclcallback = callback;
  if (!lclcallback) {
    lclcallback = function(err) {
      if (err) {
        throw err;
      }
    };
  ...
  if (typeof options !== 'object') {
    lclcallback(new Error('Options must be an object'));
    return;
  }

  if (options.queryhosts) {
    this.queryhosts = options.queryhosts;
  } else {
  ...
  if (!options.dsnObj) {
    ...
    if (options.mchost) {
      dsnObj.scheme = 'couchbase';
      dsnObj.hosts = options.mchost;
    } else if (options.srvhost) {
      dsnObj.scheme = 'http';
      dsnObj.hosts = [
        options.srvhost
      ];
      ...
      dsnObj.scheme = 'http';
      dsnObj.hosts = options.host;
    } else if (options.hosts) {
      dsnObj.scheme = 'http';
      dsnObj.hosts = options.hosts;
    }

    dsnObj.bucket = options.bucket;

    if (options.cachefile) {
      dsnObj.options.config_cache = options.cachefile;
    }

    options.dsnObj = dsnObj;
  }

  // We normalize both for consistency as well as to
  //  create a duplicate object to use
  options.dsnObj = dsn.normalize(options.dsnObj);

  var bucketDsn = dsn.stringify(options.dsnObj);
  var bucketUser = options.username;
  var bucketPass = options.password;

  this._bucket = options.dsnObj.bucket;
  try {
  ...
  if (options.dsnObj.hosts.length !== 1 ||
    ...
    dns.resolveSrv(srvPrefix + '.' + srvHost, function(err, addrs) {
      if (!err) {
        ...
        var srvDsn = dsn.stringify(options.dsnObj);
        this._cb._control.call(this._cb, CONST.LCB_CNTL_REINIT_DSN, CONST.LCB_CNTL_SET, srvDsn);
      }

      this._connect(lclcallback);
    }.bind(this));
  }
}

Bucket.prototype._connect = function(callback) {
  try {
    this._cb._connect();
  } catch (e) {
    callback(e);
 ...
 */
Bucket.prototype.on = function(event, listener) {
 ...
 */
Bucket.prototype.query = function(query, values, callback) {
 ...
 * @ignore
 */
Bucket.prototype._view = function(ddoc, name, q, callback) {
  var viewtype = '_view';
  if (q.spatial) {
    viewtype = '_spatial';
  }
  ...
  this._cb.httpRequest.call(this._cb, null, {
    path: '_design/' + ddoc + '/' + viewtype + '/' + name + '?' +
      qs.stringify(q),
 ...
 * @ignore
 */
Bucket.prototype._handleRestResponse = function(error, response, callback) {
  var body, misc = {};
  ...
  if (!body) {
    var tmpErrObj = new Error('HTTP error: Invalid JSON');
    tmpErrObj.code = CONST['ErrorCode::REST'];
    return callback(tmpErrObj, null);
  }
 ...
 * @ignore
 */
Bucket.prototype._invokeStorage = function(tgt, argList) {
 ...
 * @ignore
 */
Bucket.prototype._argHelper2 = function(target, argList) {
  if (argList.length === 2) {
 ...
 * @ignore
 */
Bucket.prototype._argHelperMerge2 = function(target, argList) {
  if (argList.length === 2) {
    target.call(this._cb, [argList[0]], null, argList[1]);
  } else {
 ...
 * @ignore
 */
Bucket.prototype._argHelperRemove = function(target, argList) {
  if (argList.length === 2) {
    target.call(this._cb, [argList[0]], null, argList[1]);
  } else {
 ...
 * @ignore
 */
Bucket.prototype._arithHelper = function(dfl, argList) {
 ...
 * @ignore
 */
Bucket.prototype._multiHelper = function(target, argList) {
 ...
 * });
 */
Bucket.prototype.touch = function(key, options, callback) {
 ...
 * });
 */
Bucket.prototype.lock = function(key, options, callback) {
 ...
 * @param {object} options
 *  @param {CAS} options.cas
 *  The CAS value returned by the previously invoked {@link Bucket#lock}
 *  operation.
 * @param {KeyCallback} callback
 *
 * @see Bucket#lock
 * @see Bucket#unlockMulti
 */
Bucket.prototype.unlock = function(key, options, callback) {
 ...
 * @see Bucket#append
 * @see Bucket#prepend
 * @see Bucket#upsertMulti
 *
 * @example
 * // Store a document in the bucket
 * db.upsert('test-key', {name:'Frank'}, function(err, result) {
 *   if (err) throw err;
 *
 *   db.get('test-key', function(err, result) {
 *     if (err) throw err;
 ...
 * db.upsert('test-key', {name: 'Bart'}, {expiry: 5}, function(err, result) {
 *   if (err) throw err;
 *
 ...
 *     user.name += 'Dog';
 *
 *     db.upsert('test-key', user, {cas: result.cas}, function(err, result) {
...
/**
 * @deprecated Use {@link Bucket#upsert} instead.
 */
Bucket.prototype.set = function(key, value, options, callback) {
...
/**
 * @deprecated Use {@link Bucket#insert} instead.
 */
Bucket.prototype.add = function(key, value, options, callback) {
...
/**
 * @deprecated Use {@link Bucket#counter} instead.
 */
Bucket.prototype.incr = function(key, options, callback) {
 ...
 * @deprecated Use {@link Bucket#counter} instead.
 */
Bucket.prototype.decr = function(key, options, callback) {
  this._arithHelper(-1, arguments);
};

/**
 * Increment or decrements a key's numeric value.
 *
 ...
 * @param {KeyCallback} callback
 *
 * @see Bucket#arithmeticMulti
 *
 * @example
 * // Increment a page counter.
 * db.counter('page-views', {initial: 0, offset: 1}, function(err, result) {
 *   if (err) throw err;
 *
 *   console.log('Page view counted');
 * });
 */
Bucket.prototype.counter = function(key, options, callback) {
  this._argHelper2(this._cb.arithmeticMulti, arguments);
};
 ...
 * @param {KeyCallback} callback
 *
 * @see Bucket#observeMulti
 * @deprecated
 */
Bucket.prototype.observe = function(key, options, callback) {
  this._argHelperMerge2(this._cb.observeMulti, arguments);
};


/**
 * Multi variant of {@link Bucket#set}.
 *
...
/**
 * @deprecated Use {@link Bucket#upsertMulti} instead.
 */
Bucket.prototype.setMulti = function(kv, options, callback) {
  this.upsertMulti.apply(this, arguments);
};

/**
 * Multi variant of {@link Bucket#add}.
 *
 * @param {Object.<string,Object>} kv
 *  @param {Mixed} kv.value
 *  @param {integer} [kv.expiry]
...
/**
 * @deprecated Use {@link Bucket#insertMulti} instead.
 */
Bucket.prototype.addMulti = function(kv, options, callback) {
  this.insertMulti.apply(this, arguments);
};

/**
 * Multi variant of {@link Bucket#replace}.
 *
 * @param {Object.<string,Object>} kv
 *  @param {Mixed} kv.value
 *  @param {CAS} [kv.cas]
...
/**
 * Multi variant of {@link Bucket#prepend}.
 *
 * @param {Object.<string,Object>} kv
 *  @param {Mixed} kv.value
 *  @param {CAS} [kv.cas]
...
/**
 * Multi variant of {@link Bucket#get}.
 *
 * @param {string[]} kv
 * @param {Object.<string,Object>} options
 *  @param {boolean} [options.spooled]
 ...
 * @see Bucket#get
 */
Bucket.prototype.getMulti = function(kv, options, callback) {
  this._multiHelper(this._cb.getMulti, arguments);
};

/**
 * Multi variant of {@link Bucket#getReplica}.
 *
 * @param {string[]} kv
 * @param {Object.<string,Object>} options
 *  @param {boolean} [options.spooled]
 ...
 * @param {MultiCallback|KeyCallback} callback
 *
 * @see Bucket#getReplica
 */
Bucket.prototype.getReplicaMulti = function(kv, options, callback) {
  this._multiHelper(this._cb.getReplicaMulti, arguments);
};

/**
 * Multi variant of {@link Bucket#touch}.
 *
 * @param {Object.<string,Object>} kv
 *  @param {CAS} [kv.cas]
 *  @param {integer} [kv.expiry]
...
/**
 * Multi variant of {@link Bucket#lock}.
 *
 * @param {string[]} kv
 * @param {Object.<string,Object>} options
 *  @param {boolean} [options.spooled]
 ...
 * @see Bucket#lock
 */
Bucket.prototype.lockMulti = function(kv, options, callback) {
  this._multiHelper(this._cb.lockMulti, arguments);
};

/**
 * Multi variant of {@link Bucket#unlock}.
 *
 * @param {Object.<string,Object>} kv
 *  @param {CAS} kv.cas
 * @param {Object.<string,Object>} options
...
/**
 * Multi variant of {@link Bucket#remove}.
 *
 ...
 * @see Bucket#remove
 */
Bucket.prototype.removeMulti = function(kv, options, callback) {
  this._multiHelper(this._cb.deleteMulti, arguments);
};

/**
 * Multi variant of {@link Bucket#incr} and {@link Bucket #decr}.
 *
 ...
 * @see Bucket#decr
 */
Bucket.prototype.arithmeticMulti = function(kv, options, callback) {
  this._multiHelper(this._cb.arithmeticMulti, arguments);
};

/**
 * Multi variant of {@link Bucket#observe}.
 *
 * @param {Object.<string,Object>} kv
 *  @param {CAS} kv.cas
 * @param {Object.<string,Object>} options
 ...
 * @see Bucket#observe
 */
Bucket.prototype.observeMulti = function(kv, options, callback) {
 ...
 * });
 */
Bucket.prototype.stats = function(key, callback) {
 ...
 * and/or when Bucket attempts to reconnect in-situ (if the current
 * connection has failed).
 *
 * @member {integer} Bucket#connectionTimeout
 * @default 5000
 */
 ...
 * @member {Mixed[]} Bucket#lcbVersion
 */
 ...
 *
 * @member {Mixed[]} Bucket#clientVersion
 */
 ...
 *
 * @member {string[]} Bucket#serverNodes
 */
Object.defineProperty(Bucket.prototype, 'serverNodes', {
  get: function() {
 ...
 */
Bucket.prototype.strError = function(code) {
 ...
 * });
 */
Bucket.prototype.view = function(ddoc, name, query) {
 ...
 */
Bucket.prototype.setDesignDocument = function(name, data, callback) {
  this._cb.httpRequest.call(this._cb, null, {
    path: '_design/' + name,
    data: _makeDDoc(data),
...
/**
 * @deprecated Use {@link Bucket#setDesignDocument} instead.
 */
Bucket.prototype.setDesignDoc = function(name, data, callback) {
  this.setDesignDocument.apply(this, arguments);
};
 ...
 * @param {DDocCallback} callback
 */
Bucket.prototype.getDesignDocument = function(name, callback) {
  this._cb.httpRequest.call(this._cb, null, {
    path: '_design/' + name,
    method: CONST.LCB_HTTP_METHOD_GET,
    lcb_http_type: CONST.LCB_HTTP_TYPE_VIEW
...
/**
 * @deprecated Use {@link Bucket#getDesignDocument} instead.
 */
Bucket.prototype.getDesignDoc = function(name, callback) {
  this.getDesignDocument.apply(this, arguments);
};
 ...
 * @param {DDocCallback} callback
 */
Bucket.prototype.removeDesignDocument = function(name, callback) {
  this._cb.httpRequest.call(this._cb, null, {
    path: '_design/' + name,
...
/**
 * @deprecated Use {@link Bucket#removeDesignDocument} instead.
 */
Bucket.prototype.removeDesignDoc = function(name, callback) {
  this.removeDesignDocument.apply(this, arguments);
};
 ...
 */
Bucket.prototype.flush = function(callback) {
  this._cb.httpRequest.call(this._cb, null, {
