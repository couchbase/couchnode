...
ViewQueryResponse.prototype._bindReq = function(req) {
  var self = this;
...
Bucket.prototype._view = function(viewtype, ddoc, name, q, callback) {
  ...
  function __httpReqCreated(err, httpReq) {
    if (err) {
      return req.emit('error', err);
    }
    req._bindReq(httpReq);
    httpReq.end();
  }
  this._maybeInvoke(this._capiRequest.bind(this), [path, 'GET', __httpReqCreated]);
