...
exports.create = function(connection, config, ready) {
    ...
    function touch(key, expiry, callback) {
        var err = requiredArgs(key, expiry, callback);
        if (err) { return callback(err); }

        err = validateKeys(key, false);
        if (err) { return callback(err); }

        if (typeof expiry != "number") {
            return callback(new Error("Expiry must be specified as a number"));
        }
    ...
    function get(key, callback, spooledCallback) {
        var err = requiredArgs(key, callback);
        if (err) { return callback(err); }

        err = validateKeys(key, true);
        if (err) { return callback(err); }
    ...
    function getAndLock(key, exptime, callback, spooledCallback) {
        ...
        if (exptime) {
            if (typeof exptime != "number") {
                return callback(new Error("Expiry should be specified as a number"));
            }
        } else {
            exptime = 0;
        }
    ...
    function set(key, doc, meta, callback) {
        ...
        var err = requiredArgs(key, doc, callback);
        if (err) { return callback(err); }
        ...
        if (typeof doc != "string") {
            return callback(new Error("Document must be provided as a string"));
        }

        if (meta.expiry && typeof meta.expiry != "number") {
            return callback(new Error("Expiration must be specified as a number"));
        }

        if (meta.flags && typeof meta.flags != "number") {
            return callback(Error("Flags must be specified as a number"));
        }

        if (meta.cas && typeof meta.cas != "object") {
            return callback(Error("Invalid cas specified"));
        }
    ...
    function incr(key, meta, callback) {
        ...
        var err = validateKeys(key, false);
        if (err) { return callback(err); }

        if (config.debug) console.log("couchbase.bucket.incr("+key+")");

        if (meta.offset && typeof meta.offset != "number") {
            return callback(new Error("Offset must be specified as an number " + typeof meta.offset));
        }
        if (meta.defaultValue && typeof meta.defaultValue != "number") {
            return callback(new Error("default value must be specified as an number"));
        }
        if (meta.expiry && typeof meta.expiry != "number") {
            return callback(new Error("Expiry must be specified as an number"));
        }
    ...
    function remove(key, meta, callback) {
        ...
        if (meta.cas && typeof meta.cas != "object") {
            return callback(new Error("Invalid cas specified"));
        }
    ...
    function view(ddoc, name, query, callback) {
        ...
        if (typeof ddoc != "string") {
            return callback(new Error("Design document must be specified as a string"));
        }

        if (typeof name != "string") {
            return callback(new Error("View must be specified as a string"));
        }
...
function validateKeys(key, allowArray) {
   if (Array.isArray(key)) {
      if (!allowArray) {
          return Error("Invalid key specified (array instead of string)");
      } else {
          for (var i = 0; i < key.length; ++i) {
              if (typeof key[i] == "object") {
                  if (!key.key) {
                      return new Error("The specified key[" + i + "] is missing key: " + JSON.stringify(key));
                  }
                  if (typeof key[i].key != "string") {
                      return new Error("The specified key[" + i + "] is not a string: " + typeof key[i]);

                  }
                  if (key[i].hashkey && typeof key[i].hashkey != "string") {
                      return new Error("The specified hashkey[" + i + "] is not a string: " + typeof key[i]);
                  }
       ...
       if (typeof key == "object") {
           if (!key.key) {
               return new Error("The specified key is missing key: " + JSON.stringify(key));
           }
           if (typeof key.key != "string") {
               return new Error("The specified key is not a string: " + typeof key);

           }
           if (key.hashkey && typeof key.hashkey != "string") {
               return new Error("The specified hashkey is not a string: " + typeof key);
           }
       } else if (typeof key != "string") {
           return new Error("The specified key (" + key + ") is not a string: " + typeof key);
       }
   }
}
...
function requiredArgs() {
    for (var i = 0; i < arguments.length; i++) {
        if (arguments[i] === undefined) {
            return new ReferenceError("missing required argument");
        }
    }
}
