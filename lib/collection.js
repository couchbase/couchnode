...
class Collection {
  ...
  async get(key, options, callback) {
    const params = this._prepareCallbackAndOptions(options, callback);

    this._checkKeyParam(key);
    this._checkTimeoutOption(params.options);
    this._checkTranscoderOption(params.options);

    if (params.options.project || params.options.withExpiry) {
      return this._projectedGet(key, params.options, params.callback);
    }

    const transcoder = this._selectTranscoder(params.options.transcoder);

    return PromiseHelper.wrap((promCb) => {
      this._conn.get(
        this._scope._name, this._name,
        key, transcoder,
        0, 0, params.options.timeout * 1000,
        (err, cas, value) => {
  ...
  async exists(key, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.exists(
        this._scope._name, this._name,
        key, params.options.timeout * 1000,
        (err, cas, exists) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
  ...
  async _getReplica(mode, key, options, callback) {
    ...
    return PromiseHelper.wrapStreamEmitter(emitter, 'replica', params.callback);
  }
  ...
  async _store(opType, key, value, options, callback) {
    ...
    this._checkKeyParam(key);
    this._checkExpiryOption(params.options);
    this._checkDuraOption(params.options);
    this._checkTimeoutOption(params.options);
    this._checkTranscoderOption(params.options);

    const transcoder = this._selectTranscoder(params.options.transcoder);
    const lcbDuraMode = _durabilityModeToLcbConst(params.options.durabilityLevel);
    const persistTo = params.options.durabilityPersistTo;
    const replicateTo = params.options.durabilityReplicateTo;

    return PromiseHelper.wrap((promCb) => {
      this._conn.store(this._scope._name, this._name,
        key, transcoder, value,
        params.options.expiry, params.options.cas,
        lcbDuraMode, persistTo, replicateTo,
        params.options.timeout * 1000, opType,
        (err, cas, token) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
  ...
  async remove(key, options, callback) {
    ...
    this._checkKeyParam(key);
    this._checkCasOption(params.options);
    this._checkDuraOption(params.options);
    this._checkTimeoutOption(params.options);
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.remove(this._scope._name, this._name,
        key, params.options.cas,
        lcbDuraMode, persistTo, replicateTo,
        params.options.timeout * 1000, (err, cas) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
          }
  ...
  async getAndTouch(key, expiry, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.get(this._scope._name, this._name,
        key, transcoder,
        expiry, 0, params.options.timeout * 1000,
        (err, cas, value) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
  ...
  async getAndLock(key, lockTime, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.get(this._scope._name, this._name,
        key, transcoder,
        0, lockTime, params.options.timeout * 1000,
        (err, cas, value) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
  ...
  async unlock(key, cas, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.unlock(
        this._scope._name, this._name,
        key, cas, params.options.timeout * 1000,
        (err, cas) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
  ...
  _prepareCallbackAndOptions(arg1, arg2) {
    let params;
    ...
    this._checkCallbackParam(params.callback);
    this._checkOptionsParam(params.options);
  ...
  async _counter(key, delta, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.counter(this._scope._name, this._name,
        key, delta, params.options.initial, params.options.expiry,
        lcbDuraMode, persistTo, replicateTo,
        params.options.timeout * 1000, (err, cas, token, value) => {
          err = errors.wrapLcbErr(err);
          if (err) {
            return promCb(err);
          }
