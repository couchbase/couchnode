...
class Collection {
  ...
  _transcoderEncode(value, transcoderOverride) {
    var transcoder = this._transcoder;
    if (transcoderOverride) {
      transcoder = transcoderOverride;
    }
    return transcoder.encode(value);
  }

  _transcoderDecode(bytes, flags, transcoderOverride) {
    var transcoder = this._transcoder;
    if (transcoderOverride) {
      transcoder = transcoderOverride;
    }
    return transcoder.decode(bytes, flags);
  }
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.get(
        ...
        (err, cas, bytes, flags) => {
          ...
          promCb(err, {
            cas: cas,
            value: this._transcoderDecode(bytes, flags)
          });
        });
    }, callback);
  }
  ...
  async _getReplica(mode, key, options, callback) {
    ...
    this._conn.getReplica(
      this._scope._name, this._name,
      key, mode, options.timeout,
      (err, rflags, cas, bytes, flags) => {
        if (err) {
          // W simply do not return items when they error directly.
          return;
        }
    ...
    return PromiseHelper.wrapStreamEmitter(emitter, callback);
  }
  ...
  async _store(opType, key, value, options, callback) {
    ...
    const [bytes, flags] = this._transcoderEncode(value, options.transcoder);
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.store(this._scope._name, this._name,
        key, bytes, flags, options.expiry, options.cas,
  ...
  async getAndLock(key, lockTime, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.get(this._scope._name, this._name,
        key, 0, lockTime, options.timeout,
        (err, cas, bytes, flags) => {
          if (err) {
            return promCb(err);
          }
  ...
  async _counter(key, delta, options, callback) {
    ...
    return PromiseHelper.wrap((promCb) => {
      this._conn.counter(this._scope._name, this._name,
        ...
        lcbDuraMode, persistTo, replicateTo,
        options.timeout, (err, cas, token, value) => {
          if (err) {
            return promCb(err);
          }
