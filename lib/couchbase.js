 ...
 *   Credentials for the bucket. This is typically the bucket name
 ...
 *
 * @param {Function}
 * callback a callback that will be invoked when the
 ...
 *
 *  @config {integer} code The error code. This is one of the codes
 ...
 *   and return the value as-is in byte form. The resultant output will
 *   be stored as a <code>Buffer</code> object.
 *
...
Connection.prototype.remove = function(key, options, callback) {
  this._argHelperMerge2(this._cb.removeMulti, arguments);
};
 ...
 *   @config {integer|string} format. Format specifier to use for storing this
 *    value. The format may be one of the following..
 *
 * @param {KeyCallback} callback
 *
 ...
 *
 *  @config {object|string|Buffer} the value to use for this key.
 ...
 * @param {object} kv
 * @param {object=} options
 * @param {MultiCallback|KeyCallback}
 *
 * @see Connection#add
 * @see Connection#setMulti
 */
...
Connection.prototype.observeMulti = function(kv, callback) {
  this._cb.observeMulti(kv, null, callback);
};
 ...
 *
 *  @config {integer} method the request method. This is one of the
 *   <code>LCB_HTTP_METHOD_*</code> constants, found in the
 *   <code>CONST</code> namespace
 *
 *  @config {integer} lcb_http_type
 ...
 *   string.
 *
 *  @config {string=} data. Data to be passed along to the server.
 *   Only valid if <code>method</code> is one of POST or PUT
 *
 *  @config {string=} content_type Content-Type header
 *
