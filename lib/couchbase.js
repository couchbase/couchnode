...
var couchnode = null;
if (!couchnode) {
  // Try to load custom built binary
  try {
    couchnode = require('bindings')('couchbase_impl');
  } catch(e) { }
}
if (!couchnode) {
  // Try to load prebuilt win x64
  try {
    couchnode = require('bindings')('../prebuilt/win/x64/couchbase_impl');
  } catch(e) { }
}
if (!couchnode) {
  // Try to load prebuilt win ia32
  try {
    couchnode = require('bindings')('../prebuilt/win/ia32/couchbase_impl');
  } catch(e) { }
}
if (!couchnode) {
  throw new Error('Failed to load couchnode native binding');
}
...
/**
 * Get a key from the a replica in your cluster.
 *
 * @param {string} key the key to retrieve
 * @param {object} [options] additional options for this operation
 *   @param {integer} options.index
 *   The index for which replica you wish to retrieve this value from,
 *   an index of -1 means to use the value of the first server to reply.
 *   @param {integer|string}options.format
 *   Instructs the library not to attempt conversion based on the flags,
 *   and to return the value in the format specified instead.
 * @param {KeyCallback} callback
...
/**
 * Get multiple keys from replicas at once.
 *
 * @param {object} kv
 * @param {object=} options
 * @param {MultiCallback|KeyCallback} callback
