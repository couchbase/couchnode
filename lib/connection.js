...
Connection.prototype._handleRestResponse = function(error, response, callback) {
  ...
  if (response.data === undefined) {
    ...
    // handled in further down the call chain, pass the response status to
    // disambiguate.
    return callback(null, null, response.status);
  }
...
/**
 * Deletes all the documents from the current bucket. The callback is called
 * when the flush is done, while done in this case means you will no longer see
 * any keys from before flush was issued but tmperrors may be present.  See
 * [flush description](http://www.couchbase.com/issues/browse/MB-11154) for
 * more information
 *
...
Connection.prototype.flush = function(callback) {
  var self = this;
  var markerKey = '__flush_marker_document__';

  // create a sential document so we can poll it later for handling flush
  self.set(markerKey, "marker", {}, function (err, res) {
    if (err) return callback(err);

    self._cb.httpRequest.call(self._cb, null, {
      path: '/pools/default/buckets/' + self._bucket + '/controller/doFlush',
      content_type: 'application/json',
      method: CONST.LCB_HTTP_METHOD_POST,
      lcb_http_type: CONST.LCB_HTTP_TYPE_MANAGEMENT
    }, function (err, data, httpStatus) {
      if (httpStatus === 200) {
        callback();
      } else if (httpStatus === 201) {
        // poll marker document
        var tid = setInterval(function () {
          self.get(markerKey, function (err, res) {
            if (err.code == CONST.LCB_KEY_ENOENT) {
              clearInterval(tid);
              callback();
            }
          })
        ...
        callback(new Error('HTTP error ' + httpStatus));
      }
    });
  })
};
