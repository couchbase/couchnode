...
var ViewFields = ['descending', 'endkey', 'endkey_docid',
  ...
  'stale', 'startkey', 'startkey_docid', 'bbox', 'spatial'];
...
function normalizeQuery(query) {
  ...
  for (var q in query) {
    if (query.hasOwnProperty(q)) {
      if (ListFields.indexOf(q) >= 0) {
        queryOut[q] = query[q].join(',');
      } else if (JsonFields.indexOf(q) >= 0) {
...
Paginator.prototype.next = function(callback) {
  if (this._nextkey && this._nextkey_docid) {
    var q = extend(extend({}, this._qobj.q),{
      ...
      startkey_docid: this._nextkey_docid
    });
    var p = this;
