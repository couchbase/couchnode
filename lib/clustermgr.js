...
function _respParseError(resp, data) {
  if (resp.statusCode < 200 || resp.statusCode >= 300) {
    ...
    if (!errData) {
      return new Error(
          'operation failed (' + resp.statusCode + ', ' + data + ')');
    }

    if (typeof errData === 'string') {
      return new Error(
          'operation failed (' + resp.statusCode + ', ' + errData + ')');
    }
...
ClusterManager.prototype.upsertUser = function(userid, settings, callback) {
  var httpReq =
      this._mgmtRequest('/settings/rbac/users/local/' + userid, 'PUT');
  httpReq.on('response', _respRead(function(err, resp, data) {
    if (err) {
      callback(err);
      return;
...
ClusterManager.prototype.removeUser = function(userid, callback) {
  var httpReq =
      this._mgmtRequest('/settings/rbac/users/local/' + userid, 'DELETE');
  httpReq.on('response', _respRead(function(err, resp, data) {
    if (err) {
      callback(err);
      return;
...
ClusterManager.prototype.getUsers = function(callback) {
  var httpReq = this._mgmtRequest('/settings/rbac/users/local', 'GET');
  httpReq.on('response', _respRead(function(err, resp, data) {
    if (err) {
      callback(err);
      return;
 ...
 * @committed
 */
ClusterManager.prototype.getUser = function(userid, callback) {
  var httpReq = this._mgmtRequest('/settings/rbac/users/local/' + userid, 'GET');
  httpReq.on('response', _respRead(function(err, resp, data) {
    if (err) {
      callback(err);
      return;
