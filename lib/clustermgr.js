 ...
 * @committed
 */
ClusterManager.prototype.upsertUser = function(domain, userid, settings, callback) {
  var httpReq =
...
ClusterManager.prototype.removeUser = function(domain, userid, callback) {
  var httpReq =
      this._mgmtRequest('/settings/rbac/users/' + domain + '/' + userid, 'DELETE');
  httpReq.on('response', _respRead(function(err, resp, data) {
    if (err) {
      callback(err);
      return;
...
ClusterManager.prototype.getUser = function(domain, userid, callback) {
  var httpReq = this._mgmtRequest('/settings/rbac/users/' + domain + '/' + userid, 'GET');
  httpReq.on('response', _respRead(function(err, resp, data) {
    if (err) {
      callback(err);
      return;
